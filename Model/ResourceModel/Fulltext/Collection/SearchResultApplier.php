...
namespace KevinPhung\SortOutOfStock\Model\ResourceModel\Fulltext\Collection;
...
class SearchResultApplier implements SearchResultApplierInterface
{
    ...
    public function apply()
    {
        ...
        if (!empty($sortOrder['price']) && $this->collection->getLimitationFilters()->isUsingPriceIndex()) {
            ...
            $this->collection->getSelect()
                ->order(
                    new \Zend_Db_Expr("is_salable DESC, price_index.min_price = 0, price_index.min_price {$sortDirection}")
                );
        } else {
            $orderList = join(',', $ids);
            $this->collection->getSelect()
